...
int main()
{
    ...
    const int climb_wall           = 2;
    const int align_parallel       = 3;
    const int veni_vidi_vici       = 4;
    ...
    float max_speed_rps2 = 0.5f; // define maximum speed that the position controller is changig the speed, has to be smaller or equal to kn * max_voltage
        ...
        {  
            ...
            switch (robot_state_actual) {
                ...
                case ROBOT_STATE_INIT:
                    ...
                    robot_state_actual = first_attack;
                    break;
                ...
                case first_attack:
                {
                    if(mechanical_button.read())
                    {
                        ...
                        positionController_M2.setDesiredRotation(10.0f);
                        robot_state_actual= climb_wall;
                        break;
                    }
                ...
                case climb_wall:
                {
                    if (positionController_M2.getRotation() >= 9.95f && positionController_M1.getRotation()>=9.95)
                    positionController_M1.setDesiredRotation(5.0f);
                    positionController_M2.setDesiredRotation(5.0f);
                    positionController_M3.setDesiredRotation(3.0f);
                    robot_state_actual = align_parallel;
                    break;
                ...
                case align_parallel:
                {
                    if (positionController_M2.getRotation() >= 9.95f && positionController_M1.getRotation()>=9.95 && positionController_M3.getRotation()>=2.9)
                    {
                        positionController_M1.setDesiredRotation(-5.0f);
                        positionController_M2.setDesiredRotation(-5.0f);
                        break;
